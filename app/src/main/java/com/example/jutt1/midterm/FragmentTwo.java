...
public class FragmentTwo extends Fragment implements PersonAdapter.OnItemClickListener {
    ...
    public static void BubbleSort (ArrayList<PersonModel> List){
        String temp;

        for (int i = 0; i < List.size() - 1; i++) {

            for (int j = 1; j < List.size() - i; j++) {
                if (Integer.parseInt(List.get(j-1).getRating_bar()) < Integer.parseInt(List.get(j).getRating_bar())) {
                    temp = List.get(j-1).getRating_bar();
                    List.get(j-1).setRating_bar(List.get(j).getRating_bar());
                    List.get(j).setRating_bar(temp);
                }
            }
        }
    }
